                ...
                clap::SubCommand::with_name("set")
                    ...
                    .subcommand(
                        clap::SubCommand::with_name("relay")
                            .about("Set the relay to use")
                            .arg(
                                clap::Arg::with_name("hostname")
                                    .help("The hostname")
                                    .required(true),
                            )
                            .about("Set the relay to use. See also 'location'."),
                    )
                    .subcommand(
...
impl Relay {
    ...
    async fn set_relay(&self, matches: &clap::ArgMatches<'_>) -> Result<()> {
        ...
        if let Some(location) = find_relay() {
            ...
            clap::Error::with_description("No matching server found", clap::ErrorKind::InvalidValue)
                .exit()
        }
    }
